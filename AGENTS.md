...
- Tests: 42 (`flutter test`) incl. no-overflow checks on small phone, tablet, landscape, and 2.0x text scale, plus a file-browser back-navigation test (back goes up one folder at a time; a folder inside a root lands on that root's contents, not the roots list).
