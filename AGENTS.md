...
- **In-app SMB / LAN playback: FULLY REMOVED from both platforms (2026-08)**. The Android in-app SMB server browser (`smb_screen.dart`, `smb_client.dart`, `SMBClient.kt`, `SmbDataSource.kt`, channel `dreamplayer/smb`, jcifs-ng) was deleted first, then the iPad flash SMB browser + playback (`SMBClient.swift`, the `smb_screen.dart`/`smb_client.dart` Dart layer, and every `AvPlayerView` SMB path) was deleted too — it was **slow** and **didn't play every video**, and WebDAV/Jellyfin/Files-app "Open with" + bookmarked folders cover local and NAS playback faster. The **Network shares** home-screen entry is gone on ALL platforms. `BufferedSMBReader.swift` STAYS (WebDAV playback still wraps it for read-ahead) and the `AetherEngineSMB` SPM product STAYS (WebDAV's `ByteRangeSource`/`WebDAVByteRangeSource` live in that module). SMB learnings are preserved in the roadmap section below for future development. **Lesson learned on-device**: jcifs-ng's streaming read size is bound by three interlocking properties (`snd_buf_size`/`rcv_buf_size`/`transaction_buf_size`, defaults 65535); raising only the first two did nothing (still ~64 KB reads / ~5 MB/s with constant ring-buffer stalls), and raising `transaction_buf_size` to 8 MiB made the NAS reject reads with `STATUS_INVALID_PARAMETER` ("The parameter is incorrect"); 1 MiB was still rejected. Do not raise buffers past what the NAS's negotiated `MaxReadSize` accepts.
...
**Status: FULLY REMOVED from both platforms (2026-08).** An Android in-app SMB v1 (browse
...
in-app SMB browser (AMSMB2 + AetherEngineSMB) shipped and was verified on-device, but it was
retired too (2026-08): slow, didn't play every video, and the audio-track-switch reopen/teardown
path could crash** (see "iOS/iPad status" below). Both iOS `SMBClient.swift` and the Dart
`smb_screen.dart`/`smb_client.dart` layers were DELETED. WebDAV/Jellyfin/Files-app "Open with" +
bookmarked folders cover local and NAS workflows faster. The complete SMB knowledge below is the
blueprint if in-app SMB ever returns.
