...
/// A saved SMB server (see `SMBClient.kt`). Passwords never cross to Dart —
/// only `hasPassword` (and decrypted ones stay in the native data source).
class SmbServer {
  const SmbServer({
    required this.id,
    required this.name,
...
/// Wraps the native `dreamplayer/smb` channel (see `SMBClient.kt`).
class SmbClient {
  SmbClient._();
  ...
  /// Remembers a share name for a server (smbj can't enumerate shares, so
  /// unusual share names must be added by hand after the auto-probe).
  ...
  /// `smb://<serverId>/<share>/<relative path>` — the native data source
  /// resolves `<serverId>` to the saved server's encrypted credentials.
